...
// *****************************************************************************
// Set the Shortcuts
int NxLeoBase::LeoBaseInterface::shortCuts(unsigned char cmd, unsigned char value)
{
  // Compose comand
  ...
  buffer[3] = 0x01;                               // request
  buffer[4] = 0x0E;                               // cmd
  buffer[5] = cmd;                             // motor_enable
  buffer[6] = value;             // data1
  buffer[7] = checkSum((unsigned char *)buffer); // checksum
  buffer[8] = 0x7D;                              // endcode
...
// *****************************************************************************
// Set the Shortcuts
int NxLeoBase::LeoBaseInterface::getEmbedSoftwareVersion()
{
  // Compose comand
...
int NxLeoBase::LeoBaseInterface::parseSenseState(unsigned char *buffer, int index)
{
  ...
  this->ir_bumper_[BACK] = (buffer[index + 1] >> 7) & 0x01;
  this->ir_bumper_[FRONT_RIGHT] = (buffer[index + 1] >> 4) & 0x01;
  this->ir_bumper_[FRONT_LEFT] = (buffer[index + 1] >> 5) & 0x01;
  this->ir_bumper_[LEFT] = (buffer[index + 1] >> 6) & 0x01;

  this->mechainical_bumper_[BACK] = (buffer[index + 1] >> 2) & 0x01;
  this->mechainical_bumper_[RIGHT] = 0;
  this->mechainical_bumper_[LEFT] = 0;
  this->mechainical_bumper_[FRONT] = (buffer[index + 1] >> 1) & 0x01;  

  this->wheel_type = (buffer[index + 2] >> 6) & 0x03;
  this->wheel_lock = (buffer[index + 2] >> 2) & 0x01;
  this->enable_bumper = (buffer[index + 2] >> 1) & 0x01;
  this->bumper_protect_status = (buffer[index + 2] >> 0) & 0x01;  
...
{
  short temp = ((short)buffer[index + 17] << 8) | buffer[index + 16];
  float gyro_yaw = (float(temp)) / 32768 * 180;
  imu_angle = gyro_yaw / 180 * 3.1415926535898;   //角度

  temp = ((short)buffer[index + 11] << 8) | buffer[index + 10];
  float vel_z = (float(temp)) / 32768 * 2000;
  imu_yaw_ = vel_z*3.1415926535898/180;   //角速度 
  return 0;
}
...
void NxLeoBase::LeoBaseInterface::parseComInterfaceData(unsigned char *buf, int len)
{
  ...
  parseLeftEncoderCounts(buf, 0);  // ����
  parseRightEncoderCounts(buf, 4); // ����
  parseWheelStatus(buf, 8);
  parseImuData(buf, 20); // imu_yaw
...
void NxLeoBase::LeoBaseInterface::calculateOdometry_new()
{
  ...
  double wp2m = WHEEL_PULSES_TO_M;
  static double last_theta = 0.0;
  if(this->wheel_type == 0x01)
  {
    wp2m = WHEEL_PULSES_TO_M/4;
  }
  double dist = ((double)encoder_counts_[RIGHT] * wp2m + (double)encoder_counts_[LEFT] * wp2m) / -2.0;
  ...
  {
    theta = ((double)encoder_counts_[RIGHT] * wp2m - (double)encoder_counts_[LEFT] * wp2m) / LEOBASE_AXLE_LENGTH;
    delta_theta = theta;
  }
  // Update odometry
  ...
  // robot_vel_[1] = 0.0;
  // robot_vel_[2] = w;
  robot_vel_[0] = (this->wheel_velocity_[RIGHT] - this->wheel_velocity_[LEFT])/2;
  robot_vel_[1] = 0.0;
  robot_vel_[2] = this->imu_yaw_;
  last_theta = theta;
}
