...
int main(int argc, char *argv[]) {
  ...
  auto moveit_visual_tools = moveit_visual_tools::MoveItVisualTools{
      node, "aubo_base", rviz_visual_tools::RVIZ_MARKER_TOPIC,
  ...
  auto const draw_trajectory_tool_path =
      [&moveit_visual_tools,
       jmg = move_group_interface.getRobotModel()->getJointModelGroup(
           "aubo_arm")](auto const trajectory) {
        moveit_visual_tools.publishTrajectoryLine(trajectory, jmg);
      };

  // Go to zero Pose
  prompt("Press 'next' in the RvizVisualToolsGui window go to zero");
  draw_title("Goto Zero");
  ...
  auto const collision_object_remove =
  ...
  auto publishPlanningSceneDiff =
      [&node](const moveit_msgs::msg::PlanningScene &planning_scene) {
  ...
  moveit_visual_tools.trigger();
  prompt("Press 'next' in the RvizVisualToolsGui window to remove object");
  draw_title("Removing");
  moveit_visual_tools.trigger();
  moveit_msgs::msg::PlanningScene planning_scene;
  planning_scene.world.collision_objects.clear();
  planning_scene.world.collision_objects.push_back(collision_object_remove);
  planning_scene.is_diff = true;

  publishPlanningSceneDiff(planning_scene);
  moveit_visual_tools.deleteAllMarkers();

  moveit_visual_tools.trigger();
  prompt("Press 'next'  to quit");
  // Shutdown ROS
  rclcpp::shutdown();
  spinner.join();
  return 0;
