...
// user-defined callback that checks whether the "shoulder_joint" of
// the AUBO robot is at a positive or negative angle:
bool stateFeasibilityTestExample(const moveit::core::RobotState& kinematic_state, bool /*verbose*/)
{
  const double* joint_values = kinematic_state.getJointPositions("shoulder_joint");
  return (joint_values[0] > 0.0);
}
// END_SUB_TUTORIAL

static const rclcpp::Logger LOGGER = rclcpp::get_logger("planning_scene_constraints");
...
int main(int argc, char** argv)
{
  ...
  auto planning_scene_constraints_node = rclcpp::Node::make_shared("planning_scene_constraints", node_options);

  rclcpp::executors::SingleThreadedExecutor executor;
  executor.add_node(planning_scene_constraints_node);
  ...
  robot_model_loader::RobotModelLoader robot_model_loader(planning_scene_constraints_node, "robot_description");
  ...
  // ~~~~~~~~~~~~~~~~~~~~
  //
  // Now, we will do collision checking only for the aubo_arm of the
  // AUBO, i.e. we will check whether there are any collisions between
  // the aubo_arm and other parts of the body of the robot. We can ask
  // for this specifically by adding the group name "aubo_arm" to the
  // collision request.

  collision_request.group_name = "aubo_arm";
  current_state.setToRandomPositions();
  collision_result.clear();
  planning_scene.checkSelfCollision(collision_request, collision_result);
  ...
  // ~~~~~~~~~~~~~~~~~~~~~~~~~~~
  //
  // First, manually set the AUBO arm to a position where we know
  // internal (self) collisions do happen. Note that this state is now
  // actually outside the joint limits of the AUBO, which we can also
  // check for directly.

  std::vector<double> joint_values = { 0.0, 0.0, -2.9, 0.0, 1.4, 0.0 };
  const moveit::core::JointModelGroup* joint_model_group = current_state.getJointModelGroup("aubo_arm");
  ...
  // have happened at a given configuration of the AUBO arm. We can ask
  ...
  // on the end-effector of the aubo_arm group of the AUBO robot. Note the
  ...
  desired_pose.header.frame_id = "aubo_base";
  moveit_msgs::msg::Constraints goal_constraint =
      kinematic_constraints::constructGoalConstraints(end_effector_name, desired_pose);
  RCLCPP_INFO_STREAM(LOGGER, "Test debug: end_effector_name is " << end_effector_name);
  ...
  bool state_valid = planning_scene.isStateValid(copied_state, kinematic_constraint_set, "aubo_arm");
