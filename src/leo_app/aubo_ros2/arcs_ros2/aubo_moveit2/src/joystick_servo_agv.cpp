...
public:
  ...
  : Node("joy_to_twist_publisher", options), frame_to_publish_(BASE_FRAME_ID)
  {
    ...
    // switch_input_client_ = this->create_client<moveit_msgs::srv::ServoCommandType>("/servo_node/switch_command_type");
    // auto request = std::make_shared<moveit_msgs::srv::ServoCommandType::Request>();
    // request->command_type = moveit_msgs::srv::ServoCommandType::Request::TWIST;
    // switch_input_client_->wait_for_service(std::chrono::seconds(1));

    // Switch input client
    switch_input_client_ = this->create_client<moveit_msgs::srv::ServoCommandType>("/servo_node/switch_command_type");
    if (!switch_input_client_->wait_for_service(std::chrono::seconds(5))) {
    ...
    if (rclcpp::spin_until_future_complete(this->get_node_base_interface(), result_future) == rclcpp::FutureReturnCode::SUCCESS) {
      ...
      if (result->success) {
        RCLCPP_INFO(this->get_logger(), "Switched to input type: Twist");
      } else {
        RCLCPP_WARN(this->get_logger(), "Could not switch input to: Twist");
      }
    } else {
      RCLCPP_ERROR(this->get_logger(), "Failed to call service /servo_node/switch_command_type");
    }
  

    // if (switch_input_client_->wait_for_service(std::chrono::seconds(1)))
    // {
    //   auto result = switch_input_client_->async_send_request(request);
